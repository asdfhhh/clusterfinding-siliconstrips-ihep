...
void Incident_vector::GetPosition(unsigned int* x,unsigned int* y)
{
	...
	*x=double((rand()%precision))*MAX_X/precision;
	*y=double((rand()%precision))*MAX_Y/precision;
...
void Incident_vector::GetDirection(double* vx, double* vy, double* vz)
{
	...
	int precision=double(RAND_MAX);
	*vx=double((rand()%precision-precision/2))/(precision/2);
	*vy=double((rand()%precision-precision/2))/(precision/2);
	*vz=double((rand()%(precision/2)))/(precision/2);
}
