...
void DmpBgoDetectorDescription::DefineMaterials()
{ 
...
void DmpBgoDetectorDescription::Construct(G4VPhysicalVolume* physi, G4LogicalVolume* logi )
{
  physiCAL = physi;
  logicCAL = logi;
  ...
  solidCALLayerX = new G4Box("CALLayerX",			
			     CALSizeXY/2,CALSizeXY/2,CALLayerThickness/2); 
  ...
  for (i = 0; i < NbOfCALLayers; i++)
    {
     ...
     //G4cout << "         Cal y-layer placed at z= " << -CALSizeZ/2+CALBarThickness/2+(i)*2*CALBarThickness << G4endl;
     //G4cout << "         Cal x-layer placed at z= " << -CALSizeZ/2+CALBarThickness/2+CALBarThickness+(i)*2*CALBarThickness << G4endl;
  ...
  solidCALDetectorY = new G4Box("CALDetectorY",			
				...
				CALBarThickness/2,CALBarLength/2,CALBarThickness/2); 
  ...
  for (i = 0; i < NbOfCALBars; i++)
    {
      ...
      //G4cout << "       Placing y-bar " << i << " at x = " << -CALSizeXY/2+ CALBarY/2 + (i)*CALBarY << " y = 0 " << G4endl;
      //G4cout << "       Placing x-bar " << i << " at x = " << 0 << " y = " << -CALSizeXY/2+ CALBarY/2 + (i)*CALBarY << G4endl;
  ...
  G4cout << "DmpBgoDetectorDescription construction completed !!! " << G4endl;
  return;
}
