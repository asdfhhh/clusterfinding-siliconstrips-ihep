...
DmpSimuEventAction::DmpSimuEventAction(DmpRootNtupleManager* ntupleMgr)
  :trackerCollID(-1),bgoHitCollID(-1),                
  psdHitCollID(-1), drawFlag("all")
{ 
...
void DmpSimuEventAction::BeginOfEventAction(const G4Event* evt)
{
  ...
  if(psdHitCollID==-1) {
    psdHitCollID =
      SDman->GetCollectionID("PSDHitCollection");
  }
  if(bgoHitCollID==-1) {
    bgoHitCollID =
      SDman->GetCollectionID("BGOHitCollection");
  }
...
void DmpSimuEventAction::EndOfEventAction(const G4Event* evt)
{
  ...
  if (HCE)
    {      
      THC = (DmpSimuStkHitsCollection*)(HCE->GetHC(trackerCollID));
